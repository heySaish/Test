...
export const Navbar: React.FC = () => {
  ...
  const navLinks = [
    { path: '/features', label: 'Features', icon: Cpu },
    { path: '/voidsu', label: 'VoidSU', icon: Shield },
    { path: '/dashboard', label: 'Dashboard', icon: Activity },
    { path: '/devices', label: 'Devices', icon: Layers },
    { path: '/downloads', label: 'Downloads', icon: Download },
    { path: '/docs', label: 'Docs', icon: BookOpen },
    { path: '/changelog', label: 'Changelog', icon: Terminal },
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-[#050508]/95 backdrop-blur-md border-b border-zinc-800/80 py-3 shadow-lg shadow-black/50'
          : 'bg-transparent py-5'
      }`}
    >
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-0 top-[65px] bg-[#08080d]/95 backdrop-blur-xl border-b border-zinc-800/90 shadow-2xl transition-all">
          <div className="max-w-7xl mx-auto px-4 py-6 space-y-2">
            <div className="grid grid-cols-2 gap-2 pb-4 border-b border-zinc-800/60">
              <NavLink
                to="/"
                className={({ isActive }) =>
                  `flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-xs font-mono transition-all ${
                    isActive ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30' : 'text-zinc-300 hover:bg-zinc-900 border border-zinc-800/40'
                  }`
                }
              >
